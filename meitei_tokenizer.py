...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python meitei_tokenizer.py <path_to_sentencepiece.model>")
        sys.exit(1)

    model_path = sys.argv[1]
    nlp = create_nlp(model_path)

    test_text = "ꯆꯦꯔꯣꯀꯤ ꯑꯁꯤ ꯌꯨꯅꯥꯏꯇꯦꯗ ꯁ꯭ꯇꯦꯠꯁꯀꯤ ꯑꯣꯀ꯭ꯂꯥꯍꯣꯃꯥꯒꯤ ꯁꯍꯔ ꯑꯃꯅꯤ ꯫"
    doc = nlp(test_text)

    print(f"Original: {test_text}")
    print(f"Tokens: {[token.text for token in doc]}")
    print(f"Spaces: {[token.whitespace_ for token in doc]}")
    print(f"Reconstructed: {doc.text}")
